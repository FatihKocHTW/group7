...
# Welche Excel-Dateien + Zeilenbereiche (wie beim Linking)
EXCEL_SPECS = [
...
# >>> RECHTE WERTE an deine Übersicht anpassen! <<<
LABELS: Dict[str, str] = {
    "t1":  "Inventar-Nr",
    "t2":  "Hersteller",
    "t3":  "Materialien",
...
def strip_parens(val):
    """Entfernt (...) und wandelt NaN/None in leere Strings."""
    if val is None:
        return ""
    s = str(val).strip()
...
def find_file_anywhere(root: str, year: str, fname: str) -> str | None:
    """Suche primär in <root>/<year>/fname, dann <root>/fname, dann rekursiv."""
...
def _parse_images_field(field: Any) -> List[str]:
    """Akzeptiert echte Listen, String-Listen '[]', oder Komma-Strings."""
...
def build_prompt_from_tcols(meta_row: Dict[str, Any]) -> str:
    """
    Prompt nutzt ausschließlich Excel-Spalten (t1..t14):
    Für jede vorhandene Spalte:  'KORREKTER NAME [Tn]: Wert'
    """
    lines = []
...
def query_chatgpt(images: List[str], prompt_text: str) -> str:
    # Begrenzung und Sichtbarkeit der tatsächlich angehängten Bilder
    to_send = images[:MAX_IMAGES_PER_CALL]
    print("   → angehängte Bilder (Count={}): {}".format(
        len(to_send), ", ".join(os.path.basename(p) for p in to_send)
    ))
...
def load_meta_rows_from_excels(excel_dir: str) -> pd.DataFrame:
    ...
    for fname, (start, stop) in EXCEL_SPECS:
        ...
        # Werte säubern (Klammern raus & NaN->"")
...
class CatalogGenerator:
    ...
    def run(self, limit: int = 1):
        ...
        for i, (_, row) in enumerate(self.linked.iterrows()):
            ...
            # Für Nachvollziehbarkeit: Prompt in Konsole zeigen
            print(f"\n--- PROMPT für {obj_id} ---\n{prompt}\n--- ENDE PROMPT ---\n")

            print(f"🔍 {obj_id}: {len(image_paths)} Bilder → Anfrage an ChatGPT …")
            try:
                # sichtbar: welche Bilder wirklich gesendet werden (in query_chatgpt nochmals geloggt)
...
if __name__ == "__main__":
    ...
    cg.run(limit=1)  # fürs Testen klein halten; später erhöhen
